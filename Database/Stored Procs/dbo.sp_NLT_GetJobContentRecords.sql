...
create proc sp_NLT_GetJobContentRecords
	@jobid int = 0
as
begin
	...
	BEGIN
		select 	@sql = 'select ''' + @tableName + ''' as TableName, * from ' + @tableName + ' where workflowstatus <> ''LIVE'' and DeploymentJobId = ' + cast(@jobid as varchar(5))
		print @sql
		exec(@sql)
